...
-(void)viewDidLoad
{
    ...
    //NSLog(@"the sharedCFE singleton is here... %@, with a count of %i",[SharedCashflowEventsArray singleton].sharedCashflowEventsArray, [SharedCashflowEventsArray singleton].sharedCashflowEventsArray.count);
    
    for (int i=0; i<[SharedCashflowEventsArray singleton].sharedCashflowEventsArray.count; i++)
    {
        NSMutableArray *paymentSeries = [[[SharedCashflowEventsArray singleton].sharedCashflowEventsArray objectAtIndex:i] paymentSeries];
        for (int x=0; x<paymentSeries.count; x++)
        {
            //NSLog(@"cfe #%i, ps #%i value = %@", i, x, [paymentSeries objectAtIndex:x]);
        }
    }
    ...
    {
        //NSLog(@"running in if statement for title = Line Chart");
...
{
    NSLog(@"running in changeValue method...");
    self.title = @"Line Chart";
    if ([self.title isEqualToString:@"Line Chart"])
    {
        NSLog(@"running in changeValue method AND inside if statement where title = Line Chart...");
        // Line Chart #1
